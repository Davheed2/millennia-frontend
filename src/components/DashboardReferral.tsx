...
export default function DashboardReferrals() {
  ...
  const { data: referral, error: queryError } = useQuery<Referrals[], Error>({
    queryKey: ["referrals"],
    queryFn: async () => {
      const { data: responseData, error } = await callApi<
        ApiResponse<Referrals[]>
  ...
  return (
    <>
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <div className="p-3 md:p-4 bg-invest/5 rounded-lg mb-6">
          ...
          {referral && referral.length === 0 ? (
            ...
            <div className="rounded-md border overflow-x-auto">
              <div className="min-w-full">
                <div className="grid grid-cols-4 gap-2 md:gap-4 p-3 md:p-4 bg-gray-50 text-xs md:text-sm font-medium text-gray-500">
                  <div>Name</div>
                  <div>Email</div>
                  <div>Date</div>
                  <div>Status</div>
                ...
                {referral &&
                  referral.map((referral) => (
                    <div key={referral.id}>
                      <div className="grid grid-cols-4 gap-2 md:gap-4 p-3 md:p-4 items-center text-xs md:text-sm">
                        <div className="truncate">
                          {referral.referreeFirstName}{" "}
                          {referral.referreeLastName}
                        </div>
